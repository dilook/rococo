...
<article class="card m-8 p-4 min-h-[90%]">
    <div class="grid grid-cols-1 lg:grid-cols-2 m-4 min-h-full">
        <img data-testid="painting-image" src={$singlePaintingStore?.data?.content} alt={$singlePaintingStore?.data?.title} class="my-4 mx-auto w-full"/>
        <div>
            <div>
                <header class="card-header text-center font-bold">{$singlePaintingStore?.data?.title}</header>
                <div id="artistName" class="text-center">{$singlePaintingStore?.data?.artist?.name}</div>
            ...
            <div id="description" class="m-4">{$singlePaintingStore?.data?.description}</div>
        </div>
    </div>
</article>